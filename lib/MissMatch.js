...
mm.target.match = (function () {
  ...
  function match(candidate, patterns, opts) {
    // >=0.1.2: patterns and handlers may also be supplied as array
      ...
      if(patterns.hasOwnProperty(pattern)) {
        ...
        if(result.result) {
          ...
          return typeof handler === 'function'
            ? ((opts && opts.arrow) ? handler(result.context) : handler.call(result.context))
            : handler;
        }
      }
    }
...
mm.target.matchJSON = function (c,p,o) {
  "use strict";
  return mm.match(JSON.parse(c), p,o);
};