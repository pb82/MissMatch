  ...
  function match(candidate, patterns) {
    for(pattern in patterns) {
      ...
      if (buffer.hasOwnProperty(pattern)) {
        ...
        match_function = makeCompiler(AST).compile();
        buffer[pattern] = match_function;
      }
            
      var result = match_function (candidate);
      if(result.result) {
...
mm.compile = function (p) {
  var AST = makeParser(p).parse();
  return makeCompiler(AST).compile();
}

mm.json = function (c,p) {
  return mm(JSON.parse(c), p);
}
