...
mm.makeParser = function (src) {
  ...
  function parseProperties(node) {
    ...
    while(true) {
       ...
       */
      if(peek() === '.') {
        parseProperty(node,false);  // property
      } else if(peek() === ':') {
  ...
  function parseStringLiteral(AST) {
    ...
    newNode(extractStringLiteral(), node.nodes);
  }
  ...
  function parseDateLiteral(AST) {
    var literal = extractStringLiteral(), node, date;
    date = new Date(literal);
        
    if(date.toString() === 'Invalid Date') {
      unexpectedTokenException('date');
    } else {
      node = newNode('equalsDate', AST);
      newNode(date, node.nodes);
    }
  }
...
mm.makeCompiler = function (AST) {
  ...
  function curry(fun, args, asArray) {
    /**
     * This is a special case of curry. We always have exactly one
     * missing parameter. Otherwise we would have to treat the
     * arguments object which is expensive.
     */
    return function (x) {
      return fun.apply(
        bindingContext, (asArray ? args : [args]).concat([x])
      );
    };
  }
  ...
  function testProperty(m,x,o,r) {
    if(m.length) {
      /**
       * An additional type descriptor for a property was specified.
    ...
    for(index=0;index<ast.length;index++) {
      ...
      switch(node.type) {
        ...
        case 'property':
          ...
          break;
        case 'prototype':
          matcher = curry(hasPrototypeProperty,
            [compileNode(node.nodes), node.name ], true);
          break;
        case 'equals':
          ...
          break;
        case 'equalsDate':
          matcher = curry(equalsDate, [node.nodes[0].type], true);
          break;
        case 'or':
