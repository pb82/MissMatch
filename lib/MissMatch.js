    ...
    if(hasNext()) {
      switch (peek()) {
        ...
        case 'o':
          ...
          break;
        case 'n':
          parseGeneric(AST,'n');    // numeric
          break;
        case 's':
          parseGeneric(AST,'s');    // string
          break;
        case 'b':
          parseGeneric(AST,'b');    // boolean
          break;
        case 'f':
          parseGeneric(AST,'f');    // function
          break;
        case 'd':
          parseGeneric(AST,'d');    // date
          break;
        case 'r':
          parseGeneric(AST,'r=');   // regexp object
          break;                  
        case '_':
          parseGeneric(AST, '_');   // any
          break;
        default:
...
/*--------------------------------------------------------------------*/
commonjs = (typeof window === 'undefined' && exports) ? exports : {};

commonjs.match = mm.match = (function () {
  "use strict";
...
commonjs.compile = mm.compile = function (p) {
  return mm.makeCompiler(mm.makeParser(p).parse()).compile();
};

commonjs.matchJson = mm.matchJSON = function (c,p) {
  return mm.match(JSON.parse(c), p);
};

commonjs.matchArgs = mm.matchArgs = function (p) {
